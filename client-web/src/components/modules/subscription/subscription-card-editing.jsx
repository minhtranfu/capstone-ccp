...
class SubscriptionCardEditing extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      subscription: {
        id: subscription.id,
        beginDate: moment(subscription.beginDate),
        endDate: moment(subscription.endDate),
        latitude: subscription.latitude,
        longitude: subscription.longitude,
        maxDistance: subscription.maxDistance,
        maxPrice: subscription.maxPrice,
        address: subscription.address,
        equipmentType: {
          id: subscription.equipmentType.id
        }
      }
    };
  }
  ...
  _handleUpdated = () => {
    ...
    onUpdated && onUpdated(this.updatedSubscription);
  };
  ...
  _handleSubmitForm = async e => {
    ...
    try {
      ...
      const updateResult = await subscriptionServices.updateSubscription(id, subscription);
      this.setState({
        idFetching: false
      }, () => {
        const { onUpdated } = this.props;
        onUpdated && onUpdated(updateResult);
      });
    } catch (error) {
...
export default connect(mapStateToProps, mapDispatchToProps)(SubscriptionCardEditing);
